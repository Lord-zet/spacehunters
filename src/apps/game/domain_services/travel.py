...
def calculate_distance(source: Coordinates, target: Coordinates) -> int:
    galaxy_gap = abs(source.galaxy - target.galaxy)
    system_gap = abs(source.system - target.system)
    position_gap = abs(source.position - target.position)
